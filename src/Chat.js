...
export default class Chat extends Component {
   ...
   async sendmsg(){
      await fetch('http://127.0.0.1:8000/message/',{
          method:'POST',
          body:JSON.stringify({"user":this.props.match.params.user,"messages":this.myRef.current.value,"group":this.props.match.params.group}),
          headers:{
  ...
  clearInterval(this.interval);
    ...
    render() {
      ...
      const groups=this.state.group.map((grp)=>{
        ...
        return(
          <a href={"/Chat/"+grp.gpName+"/"+localStorage.getItem("username")} className="list-group-item list-group-item-action list-group-item-light rounded-0">
            ...
            return(
                  ...
                  <div className="media-body">
                    <div className="bg-primary rounded py-2 px-3 mb-2">
                      <p className="text-small mb-0 text-white">{msg.messages}</p>
                    </div>
                ...
                <form  className="bg-light">
                  <div className=" input-group" style={{  display: 'flex',backgroundColor: 'white'}} >
                    ...
                    <div className="input-group-append">
                      <button id="button-addon2"  onClick={this.sendmsg} type="button" className="btn btn-link"> <i className="fa fa-paper-plane"></i></button>
                    </div>
                  </div>
                </form>
              </div>
